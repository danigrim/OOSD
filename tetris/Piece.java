...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		skirt = new int[rightmost+1]; 
		Arrays.fill(skirt, topmost);
			for(TPoint coordinate: body) {
					if(skirt[coordinate.x]>coordinate.y) {
						skirt[coordinate.x] = coordinate.y;
				}	
			}
		height = topmost+1;
		width = rightmost+1;
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint new_coordinates[] = new TPoint[this.body.length];
		for(int i=0; i<body.length; i++) {
			TPoint curr = new TPoint(body[i]);
			new_coordinates[i] = curr;
		}
		for(TPoint coordinate: new_coordinates) {
			int new_x = this.height-coordinate.y-1;
			int new_y = coordinate.x;
			coordinate.x= new_x;
			coordinate.y = new_y;
		}
		
		return new Piece(new_coordinates);
	...
	*/
	public boolean equals(Object obj) {
		...
		for(TPoint coordinate_original: this.body) {
			num_points ++;
			for(TPoint coordinate_other: other.body) {
				if(coordinate_original.equals(coordinate_other)) {
					equal_points++;
				}
			}
		}
		
		if(equal_points < (num_points)) {
			return false;
		}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece curr_piece = root; 
		Piece next_piece;
		while(true) {
			next_piece = curr_piece.computeNextRotation();
			if(root.equals(next_piece)) {
				curr_piece.next=root;
				break;
			}
			curr_piece.next = next_piece;
